...
func (s *sessionSuite) TestAddPackageSessionFiles(c *C) {
	...
	err = wrappers.AddSnapSessionFiles(info)
	c.Assert(err, IsNil)
...
func (s *sessionSuite) TestAddPackageSessionFilesCleanup(c *C) {
	reset := release.MockOnCoreDesktop(true)
	defer reset()
...
func (s *sanitizeSessionFileSuite) TestSanitizeIgnoreNotWhitelisted(c *C) {
