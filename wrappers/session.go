...
// AddSnapSessionFiles puts in place the session files from the snap.
func AddSnapSessionFiles(s *snap.Info) (err error) {
	...
	if err != nil {
		return fmt.Errorf("cannot get wayland sessions for %v: %s", baseDir, err)
	}
	...
	for _, df := range sessionFiles {
		...
		installedWaylandSessionFileName := filepath.Join(dirs.SnapWaylandSessionsDir, fmt.Sprintf("%s_%s", s.DesktopPrefix(), filepath.Base(df)))
		content = sanitizeSessionFile(s, installedWaylandSessionFileName, content)
		if err := osutil.AtomicWriteFile(installedWaylandSessionFileName, content, 0755, 0); err != nil {
			return err
		}
		created = append(created, installedWaylandSessionFileName)
	}
...
func RemoveSnapSessionFiles(s *snap.Info) error {
	sessionFiles, err := filepath.Glob(filepath.Join(dirs.SnapWaylandSessionsDir, fmt.Sprintf("%s_*.desktop", s.DesktopPrefix())))
	if err != nil {
		return nil
	}
	for _, df := range sessionFiles {
